...
use config::{ServerConfig, ClientConfig, WireGuardConfig};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Init { endpoint, port, network, interface } => {
            init_server(&cli.output, &endpoint, port, &network, &interface, cli.verbose)
        }
        Commands::Client { name, ip, full_tunnel, #[cfg(feature = "qr")] qr } => {
            add_client(&cli.output, &name, ip, full_tunnel, #[cfg(feature = "qr")] qr, cli.verbose)
        }
...
fn show_server(output_dir: &PathBuf) -> Result<()> {
    ...
    println!("  Endpoint: {}:{}", server_config.endpoint, server_config.port);
